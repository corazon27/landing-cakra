...
@section('title', 'Edit Portfolio: ' . $portfolio->judul)
    ...
    <form action="{{ route('admin.portfolio.update', $portfolio->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <select name="filter" required
                        ...
                        <option value="pos" {{ old('filter', $portfolio->filter) == 'pos' ? 'selected' : '' }}>Kasir / POS</option>
                        <option value="company" {{ old('filter', $portfolio->filter) == 'company' ? 'selected' : '' }}>Company Profile
                        </option>
                        <option value="katalog" {{ old('filter', $portfolio->filter) == 'katalog' ? 'selected' : '' }}>E-Commerce / Katalog
                        </option>
                        <option value="erp" {{ old('filter', $portfolio->filter) == 'erp' ? 'selected' : '' }}>Sistem Kustom / ERP</option>
                    </select>
                </div>
                ...
                <div>
                    <label class="block text-xs font-bold uppercase text-slate-600 tracking-wider mb-2">Label Kategori
                        Tampilan</label>
                    <input type="text" name="kategori" value="{{ old('kategori', $portfolio->kategori) }}"
                ...
                <div>
                    <label class="block text-xs font-bold uppercase text-slate-600 tracking-wider mb-2">Nama Klien /
                        Industri</label>
                    <input type="text" name="klien_industri" value="{{ old('klien_industri', $portfolio->klien_industri) }}"
                ...
                <div>
                    <label class="block text-xs font-bold uppercase text-slate-600 tracking-wider mb-2">Durasi
                        Pengerjaan</label>
                    <input type="text" name="durasi" value="{{ old('durasi', $portfolio->durasi) }}"
            ...
            <div>
                ...
                @if($portfolio->gambar)
                <div class="mb-3">
                    <img src="{{ asset('storage/portofolio/' . $portfolio->gambar) }}" alt="Mockup {{ $portfolio->judul }}"
                        class="w-64 aspect-video object-cover rounded-xl border border-slate-200">
                    <p class="text-xs text-slate-400 mt-1.5"><i class="fas fa-info-circle mr-1"></i>Kosongkan pilihan
                        file jika tidak ingin mengubah gambar.</p>
                </div>
                @endif
            ...
            <div>
                ...
                <textarea name="deskripsi_singkat" rows="3" required
                    ...
                    class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition outline-none resize-none">{{ old('deskripsi_singkat', $portfolio->deskripsi_singkat) }}</textarea>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-6">
            ...
            <div x-data="{ items: {{ json_encode(old('pain_points', $portfolio->pain_points ?? [''])) }} }">
            ...
            <div x-data="{ items: {{ json_encode(old('tech_stack', $portfolio->tech_stack ?? [''])) }} }">
            ...
            <div x-data="{ items: {{ json_encode(old('fitur_utama', $portfolio->fitur_utama ?? [''])) }} }">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <label class="block text-[10px] font-bold uppercase text-slate-500 tracking-wider mb-1.5">Statistik
                        1</label>
                    <input type="text" name="impact_stat_1" value="{{ old('impact_stat_1', $portfolio->impact_stat_1) }}"
                    ...
                    <input type="text" name="impact_desc_1" value="{{ old('impact_desc_1', $portfolio->impact_desc_1) }}"
                ...
                <div class="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <label class="block text-[10px] font-bold uppercase text-slate-500 tracking-wider mb-1.5">Statistik
                        2</label>
                    <input type="text" name="impact_stat_2" value="{{ old('impact_stat_2', $portfolio->impact_stat_2) }}"
                        placeholder="Contoh: 4x Lipat" required
                        class="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm font-bold text-blue-600 outline-none mb-2">
                    <input type="text" name="impact_desc_2" value="{{ old('impact_desc_2', $portfolio->impact_desc_2) }}"
                ...
                <div class="bg-slate-50 p-4 rounded-xl border border-slate-100">
                    <label class="block text-[10px] font-bold uppercase text-slate-500 tracking-wider mb-1.5">Statistik
                        3</label>
                    <input type="text" name="impact_stat_3" value="{{ old('impact_stat_3', $portfolio->impact_stat_3) }}" placeholder="Contoh: 0%"
                        required
                        class="w-full bg-white border border-slate-200 rounded-lg px-3 py-2 text-sm font-bold text-blue-600 outline-none mb-2">
                    <input type="text" name="impact_desc_3" value="{{ old('impact_desc_3', $portfolio->impact_desc_3) }}"
            ...
            <div class="pt-2 space-y-4">
                <div>
                    ...
                    <textarea name="testimoni_konten" rows="3"
                        ...
                        class="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm focus:bg-white focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition outline-none resize-none">{{ old('testimoni_konten', $portfolio->testimoni_konten) }}</textarea>
                </div>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-600 tracking-wider mb-2">Nama Pemberi
                            Testimoni</label>
                        <input type="text" name="testimoni_nama" value="{{ old('testimoni_nama', $portfolio->testimoni_nama) }}"
                    ...
                    <div>
                        <label class="block text-xs font-bold uppercase text-slate-600 tracking-wider mb-2">Jabatan /
                            Posisi</label>
                        <input type="text" name="testimoni_jabatan" value="{{ old('testimoni_jabatan', $portfolio->testimoni_jabatan) }}"
